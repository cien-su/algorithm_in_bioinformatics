...
def trace_back(res: np.array, l1: list, l2: list, match, mismatch, gap):
    ...
    # find the largest value in matrix and put in stack
    i = np.where(res == np.max(res))[0][0]
    j = np.where(res == np.max(res))[1][0]
    m_stack = [(i, j)]  # main stack
    ...
    while m_stack:  # when main stack is not empty
        ...
        if res[row-1][col-1] == res[row-1][col] == res[row][col-1] == 0:
            # check if all the neighbours are 0s. If yes, then store the path and pop the top of main stack
            path.append(m_stack.copy())
            m_stack.pop()
        else:
            # If not, then check if main stack is longer than the other.
            if len(m_stack) > len(a_stack):
                # If yes, update the assistant stack
                a_stack.append([])
...
def sw(seq1: str, seq2: str, match, mismatch, gap, plot=False, plot_val=False):
    ...
    for i in path:
        ...
        print(seq_match2)
        print("--------------------")
    print("%s results, scores: %s"%(len(path), np.max(res_matrix)))
